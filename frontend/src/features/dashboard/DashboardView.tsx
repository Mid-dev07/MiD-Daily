  ...
  return (
    <section className="dashboard-grid page-enter">
      ...
      <div className="stat-row">
        ...
        <StatCard label="Expense" value={currency.format(expense)} hint="all recorded expenses" />
      </div>
      ...
      <section className="content-card schedule-card motion-card">
        ...
        <div className="schedule-list">
          {schedule.length === 0 ? <div className="empty-state"><strong>No activities planned</strong><span>Your Schedule is currently empty.</span></div> : schedule.map((item) => <div className="schedule-item" key={item.id}><div className="schedule-time"><strong>{item.startTime}</strong><span>{item.endTime}</span></div><div className="timeline-dot" /><div className="schedule-copy"><strong>{item.title}</strong><span>{item.type} · {item.location || 'No location'}</span></div></div>)}
        </div>
      </section>
