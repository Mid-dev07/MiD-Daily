...
const emptyDraft = (date: string): ScheduleDraft => ({
  title: '', type: 'PERSONAL', date, startTime: '09:00', endTime: '10:00', location: '', notes: '',
...
export function ScheduleForm({ open, initialItem, defaultDate, onClose, onSubmit }: ScheduleFormProps) {
  ...
  const updateRecurrence = (field: keyof ScheduleDraft['recurrence'], value: string | number) => {
      ...
      <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="schedule-form-title">
        ...
        <form className="schedule-form" onSubmit={submit}>
          ...
          <div className="form-section">
            <div className="form-section-heading"><strong>Repeat</strong><span>Recurrence stays local until backend sync.</span></div>
            <div className="form-grid two">
            ...
            {draft.recurrence.frequency !== 'NONE' && (
              <label>Repeat until<input type="date" value={draft.recurrence.until ?? ''} onChange={(e) => updateRecurrence('until', e.target.value)} /></label>
            )}
          </div>
