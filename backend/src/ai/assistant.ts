...
async function executeTool(userId: string, name: string, rawArguments: string) {
  ...
  if (name === 'create_activity') {
    ...
    if (mode === 'FLEXIBLE') {
      ...
      const validatedTargetCount = targetCount
      const validatedDurationMinutes = durationMinutes
    ...
    return {
      created: await createSchedule(userId, {
        ...
        targetCount: mode === 'FLEXIBLE' ? validatedTargetCount : targetCount,
        targetPeriod,
        durationMinutes: mode === 'FLEXIBLE' ? validatedDurationMinutes : durationMinutes,
        preferredStartTime,
        preferredEndTime,
        activityDeadline,
